...
- **2026-05-04 (v5.3):** Roster portraits — Donna delivered 7 head-shots (Rod, Jose, Angel, Mario, Alias, Tanner, Gonzo, all 688×688 AVIF) sourced from her team. Stored at `assets/team/[name].avif`. Roster grid expanded from `1fr auto` to `64px 1fr auto` (48px on mobile). Photos: 3px radius, 1px graphite outline, slight desaturation (`saturate(0.92)`) so they don't fight the heritage palette. Missing photos (Jeff, Pedro, Karess, Eddy) get a graphite tile with the first initial in Big Shoulders 900 — keeps grid alignment without faking a photo.
- **2026-05-04 (v5.4):** Donna pushed for **luxury pricing positioning** + delivered 8 work photos. Replaced **The Parade** word marquee with **The Work** photo gallery — keeps section count at 5; the marquee was abstracting the same content the photos now show concretely. Editorial 6-col grid: GT3 feature (3:2 with `object-position: center 30%` to keep the shop sign visible), then Austin-Healey + '66 Mustang half-width, then '77 Jeep + VW Bug + VW Deluxe Bus thirds. Captions: car name + small green tag (Custom paint / Restoration / Classic / Vintage). Hero meta broadened from "VW Restoration" to "Restoration & Paint" since the gallery now shows non-VW work. The 2 small Donna sent (RESTORATIONS.avif, Full car restoration.avif at 242×242) held back — too low-res for feature placement; need higher-res versions to use. The GT3 photo is unmistakably parked in front of Mr Spot's actual signage — strongest "real customer drove their GT3 here" signal we could ask for. Considered captioning paint as "Python Green" (likely correct PTS) but pulled it under no-fabrication rule; Donna can confirm with Rod and we'll add specifics later.
